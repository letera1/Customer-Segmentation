...
export default function Reports() {
  ...
  const generateReport = (reportId: string, reportName: string) => {
    ...
    setTimeout(() => {
      ...
      // Auto download
      const content = `${reportName}\nGenerated: ${newReport.date}\n\nThis is a sample report generated by SegmentAI Pro.`;
      const blob = new Blob([content], { type: "text/plain" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${reportName.replace(/\s+/g, "-")}-${Date.now()}.txt`;
      a.click();
            ...
            {reports.map((report) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={generating || report.status !== "ready"}
                  className="w-full rounded-lg bg-slate-800/50 py-2 text-sm font-medium hover:bg-slate-800 transition-colors disabled:opacity-50"
                >
                ...
                <div key={i} className="flex items-center justify-between p-4 hover:bg-slate-800/30 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="rounded-lg bg-slate-800/50 px-4 py-2 text-sm hover:bg-slate-800"
                    >
                      Download
                    </button>
                  </div>
